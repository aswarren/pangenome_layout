 ...
 * <ul>
 * <li><b>Nodes:</b> ID and LABEL</li>
 * <li><b>Edges:</b> SOURCE, TARGET and WEIGHT</li>
 * </ul>
...
public class MYSQLImportExport {
	...
	public void script() {
		...
		// Connect database
		String url = SQLUtils.getUrl(db.getSQLDriver(), db.getHost(),
				db.getPort(), db.getDBName());
		Connection connection = null;
		try {
			// System.err.println("Try to connect at " + url);
			connection = db.getSQLDriver().getConnection(url, db.getUsername(),
					db.getPasswd());
		...
		for (Node node : graph.getNodes().toArray()) {
			...
			String query = "UPDATE " + db.getDBName() + ".nodes SET x = '" + x
					+ "', y = '" + y + "' WHERE nodes.id='" + id + "'";
			try {
