...
public class GenerateGraph {
	...
	public void script() {
		...
		Container container2 = Lookup.getDefault()
				.lookup(ContainerFactory.class).newContainer();
		...
		importController.process(container2, new AppendProcessor(), workspace); // Use
																				// AppendProcessor
																				// to
																				// append
																				// to
		...
		Container container3 = Lookup.getDefault()
				.lookup(ContainerFactory.class).newContainer();

		DynamicGraph dynamicGraph = new DynamicGraph();
		dynamicGraph.generate(container3.getLoader());
		importController
				.process(container3, new DefaultProcessor(), workspace2);
	}
}
