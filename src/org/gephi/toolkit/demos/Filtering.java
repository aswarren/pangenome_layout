 ...
 * <ul>
 * <li>Filter degrees, remove nodes with degree < 10</li>
	...
	public void script() {
		...
		try {
			File file = new File(getClass().getResource(
					"/org/gephi/toolkit/demos/resources/polblogs.gml").toURI());
		...
		degreeFilter.setRange(new Range(10, Integer.MAX_VALUE)); // Remove nodes
																	// with
																	// degree <
																	// 10
		...
		System.out.println("Nodes: " + graph.getNodeCount() + " Edges: "
				+ graph.getEdgeCount());

		// Filter, keep partition 'Blogarama'. Build partition with 'source'
		// column in the data
		...
		Partition p = partitionController.buildPartition(attributeModel
				.getNodeTable().getColumn("source"), graph);
		...
		graphModel.setVisibleView(view2); // Set the filter result as the
											// visible view
		...
		// Combine two filters with AND - Set query and query2 as sub-query of
		// AND
		...
		graphModel.setVisibleView(view3); // Set the filter result as the
											// visible view
