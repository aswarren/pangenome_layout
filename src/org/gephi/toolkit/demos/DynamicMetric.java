...
public class DynamicMetric {
	...
	public void script() {
		...
		Container container = Lookup.getDefault()
				.lookup(ContainerFactory.class).newContainer();
		...
		AttributeColumn dateColumn = attributeModel.getNodeTable().addColumn(
				"date", AttributeType.INT);
		...
		for (Node n : graph.getNodes()) {
			...
			n.getNodeData().getAttributes()
					.setValue(dateColumn.getIndex(), randomDataValue);
		}

		// Use the Data laboratory merge strategy to convert this Integer column
		// to the TimeInterval column
		AttributeColumnsMergeStrategiesController dataLabController = Lookup
				.getDefault().lookup(
						AttributeColumnsMergeStrategiesController.class);
		dataLabController.mergeNumericColumnsToTimeInterval(
				attributeModel.getNodeTable(), dateColumn, null, 1990, 2010);

		// Use the DynamicModel dynamic graph factory
		DynamicModel dynamicModel = Lookup.getDefault()
				.lookup(DynamicController.class).getModel();
		DynamicGraph dynamicGraph = dynamicModel.createDynamicGraph(graph);

		// Get the number of nodes for each period of one year - and store it in
		...
		for (int i = 1990; i < 2009; i++) {
			...
			numberofNodes = new DynamicInteger(numberofNodes,
					new Interval<Integer>(low, high, count)); // DynamicInteger
																// is immutable
		}

		// Get all intervals and print values
		System.out.println("Number of nodes:");
		for (Interval<Integer> interval : numberofNodes.getIntervals(
		...
		// Create a Degree metric to get the average degree for each time
		// interval
		Degree degree = new Degree();

		// Compute the metric for each subgraph and put result in a
		// DynamicDouble
		...
		for (int i = 1990; i < 2009; i++) {
			...
			averageDegree = new DynamicDouble(averageDegree,
					new Interval<Double>(low, high, result));
		}

		// Get all intervals and print values
		System.out.println("Average degree:");
		for (Interval<Double> interval : averageDegree.getIntervals(
				Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY)) {
			int low = (int) interval.getLow();
			int high = (int) interval.getHigh();
			System.out.println(low + "-" + high + "  ->  "
