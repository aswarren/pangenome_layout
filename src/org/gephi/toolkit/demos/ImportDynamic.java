	...
	public void script() {
		...
		try {
			File file = new File(getClass().getResource(
					"/org/gephi/toolkit/demos/resources/timeframe1.gexf")
					.toURI());
		...
		// Initialize the DynamicProcessor - which will append the container to
		// the workspace
		DynamicProcessor dynamicProcessor = new DynamicProcessor();
		dynamicProcessor.setDateMode(false); // Set 'true' if you set real dates
												// (ex: yyyy-mm-dd), it's double
												// otherwise
		dynamicProcessor.setLabelmatching(true); // Set 'true' if node matching
													// is done on labels instead
													// of ids
		...
		try {
			File file = new File(getClass().getResource(
					"/org/gephi/toolkit/demos/resources/timeframe2.gexf")
					.toURI());
			container = importController.importFile(file);
		} catch (Exception ex) {
			ex.printStackTrace();
		...
		try {
			File file = new File(getClass().getResource(
					"/org/gephi/toolkit/demos/resources/timeframe3.gexf")
					.toURI());
			container = importController.importFile(file);
		} catch (Exception ex) {
			ex.printStackTrace();
		...
		for (Node n : graph.getNodes()) {
			DynamicInteger value = (DynamicInteger) n.getNodeData()
					.getAttributes().getValue("price");
		...
		for (Node n : graph.getNodes()) {
			...
			Integer priceFrom2007to2009Avg = value.getValue(new Interval(2007,
					2009), Estimator.AVERAGE);
			System.out.println("With AVERAGE estimator: '"
					+ n.getNodeData().getLabel() + "': "
					+ priceFrom2007to2009Avg);

			Integer priceFrom2007to2009Max = value.getValue(new Interval(2007,
					2009), Estimator.MAX);
			System.out.println("With MAX estimator: '"
					+ n.getNodeData().getLabel() + "': "
		...
		FilterBuilder[] builders = Lookup.getDefault()
				.lookup(DynamicRangeBuilder.class).getBuilders();
		DynamicRangeFilter dynamicRangeFilter = (DynamicRangeFilter) builders[0]
				.getFilter(); // There is only one TIME_INTERVAL column, so it's
								// always the [0] builder
		...
		AttributeColumn priceCol = attributeModel.getNodeTable().getColumn(
				"price");
		...
		// Set the filters parameters - Keep nodes between 2007-2008 which have
		// average price >= 7
		...
		System.out.println("Node 3 in the filtered graph: "
				+ filteredGraph.contains(graph.getNode("3")));
	}
}
